...
test("all 91 tools are explicitly classified and classifications are disjoint", () => {
  const actual = new Set(registeredToolNames());
  const classified = new Set([...GOOGLE_ADS_READ_TOOL_NAMES, ...GOOGLE_ADS_WRITE_TOOL_NAMES]);
  assert.equal(actual.size, 91);
  assert.equal(GOOGLE_ADS_READ_TOOL_NAMES.size, 34);
  assert.equal(GOOGLE_ADS_WRITE_TOOL_NAMES.size, 57);
